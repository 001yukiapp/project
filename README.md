...
プロジェクト進捗をスマホで見やすく整理し、アイデア、GitHubリポジトリ、ChatGPT/Codexへの依頼文までまとめて扱う軽量PWAです。
...
- 全プロジェクトの進捗を1画面で確認
- トップに「今日の集中」を表示
- プロジェクト → フェーズ → タスクのツリー方式で管理
- チェックボックスでタスク完了
- 今日やる、作業中、詰まり中、高優先度を切り替え
- プロジェクトごとの目的、次の一手、コスト上限、リンク管理
- タスク追加・編集・削除
- 右下の「＋アイデア」から思いつきをすぐ保存
- アイデア箱で編集・削除・タスク化
- GitHubリポジトリ箱で公開リポジトリを管理
- `001yukiapp` の公開リポジトリをGitHub APIから取得
- リポジトリごとのCodex依頼文コピー
- JSONバックアップ / インポート
- ツリー方式のChatGPT共有用テキストの自動生成
- 進捗TXT保存
- localStorage保存
...
「ChatGPT連携」のコピーを押して、このチャットに貼ると、今日の集中、詰まり中、プロジェクトツリー、アイデア箱、GitHubリポジトリ箱を前提に次の判断をできます。
...
GitHubリポジトリ箱では、`GitHubから取得` で `001yukiapp` の公開リポジトリを取得できます。トークンやAPIキーは使わず、公開APIだけを使います。各リポジトリはプロジェクト/フェーズに紐づけでき、Codex依頼文もコピーできます。
...
既存データがあるブラウザでも、新しいプロジェクト、フェーズ情報、タスク、アイデア箱、GitHubリポジトリ箱はマイグレーションで追加されます。反映されない場合は、アプリ内のJSONバックアップを取ってから初期化するか、ブラウザのlocalStorageに古いデータが残っていないか確認してください。
